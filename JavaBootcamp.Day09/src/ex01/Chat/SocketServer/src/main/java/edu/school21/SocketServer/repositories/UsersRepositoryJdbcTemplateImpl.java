...
public class UsersRepositoryJdbcTemplateImpl implements UsersRepository {
    ...
    public Optional<User> findByLogin(String login) {
        ...
        return this.namedParameterJdbcTemplate.query(
                "SELECT * FROM day09ex01_users WHERE login = :login",
                paramsMap, new BeanPropertyRowMapper<>(User.class)).stream().findAny();
    }
    
    @Override
    public Optional<User> findById(Long id) {
        return this.jdbcTemplate.query("SELECT * FROM day09ex01_users WHERE id = ?",
                new Object[]{id}, new BeanPropertyRowMapper<>(User.class)).stream().findAny();
    }
    
    @Override
    public List<User> findAll() {
        return this.jdbcTemplate.query("SELECT * FROM day09ex01_users",
                new BeanPropertyRowMapper<>(User.class));
    }
    
    @Override
    public void save(User entity) {
        String sql = "INSERT INTO day09ex01_users (login, password, authorized) " +
                "VALUES (?, ?, ?)";
    ...
    public void update(User entity) {
        this.jdbcTemplate.update(
                "UPDATE day09ex01_users SET login = ?, password = ?," +
                        " authorized = ? WHERE id = ?",
    ...
    public void delete(Long id) {
        this.jdbcTemplate.update("DELETE FROM day09ex01_users WHERE id = ?", id);
    }
}